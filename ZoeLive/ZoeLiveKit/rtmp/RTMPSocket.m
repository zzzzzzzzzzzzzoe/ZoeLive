...
}@property (nonatomic, strong) RtmpStreamConfig *stream;
...
@property (nonatomic, strong) NSMutableArray <RTMPFrame *>*frameList;
@property (nonatomic, strong) NSMutableArray <RTMPFrame *>*list;
...
-(NSInteger)JF_RTMP264_Connect:(char *)push_url{
    ...
    _isConnecting = YES;
    // 代理回调网络状态
    if (self.delegate && [self.delegate respondsToSelector:@selector(socketStatus:)]) {
        [self.delegate socketStatus:ZoeRtmpState_Pending];
    }
    ...
    if(self.delegate && [self.delegate respondsToSelector:@selector(socketStatus:)]){
        [self.delegate socketStatus:ZoeRtmpState_Start];
    }
...
Failed:
    ...
    [self resetParameter];
    if(self.delegate && [self.delegate respondsToSelector:@selector(socketStatus:)]){
        [self.delegate socketStatus:ZoeRtmpState_rror];
    }
    return -1;
}
...
- (void)sendFrame {
    if (!self.isSending && self.list.count > 0) {
        ...
        RTMPFrame *frame = [self popFirstObject];
        if ([frame isKindOfClass:[RTMPVideoFrame class]]) {  // RTMP 发送视频
...
- (void)appendObject:(RTMPFrame *)frame {
    if (!frame) {
        return;
    }
    ...
    if(self.frameList.count < defaultFrameListMaxCount){
        ...
        /// 添加至缓冲区
        RTMPFrame *firstFrame = [self.frameList JF_PopFirstObject];
        if(firstFrame) [self.list addObject:firstFrame];
    }
    dispatch_semaphore_signal(_lock);
}

- (RTMPFrame *)popFirstObject{
    dispatch_semaphore_wait(_lock, DISPATCH_TIME_FOREVER);
    RTMPFrame *firstFrame = [self.list JF_PopFirstObject];
    dispatch_semaphore_signal(_lock);
    return firstFrame;
}
...
NSInteger frameDataCompare(id obj1, id obj2, void *context){
    RTMPFrame * frame1 = (RTMPFrame*) obj1;
    RTMPFrame *frame2 = (RTMPFrame*) obj2;
...
- (void)removeExpireFrame{
    ...
    RTMPFrame *firstIFrame = [self firstIFrame];
    if(firstIFrame){
...
- (NSArray*)expirePFrames{
    ...
    for(NSInteger index = 0;index < self.list.count;index++){
        RTMPFrame *frame = [self.list objectAtIndex:index];
...
- (RTMPFrame *)firstIFrame{
    for(NSInteger index = 0;index < self.list.count;index++){
        RTMPFrame *frame = [self.list objectAtIndex:index];
        if([frame isKindOfClass:[RTMPVideoFrame class]] && ((RTMPVideoFrame*)frame).isKeyFrame){
            return frame;
        }
    }
    return nil;
